...
async def test_usecases_get_should_return_not_found():
    with pytest.raises(NotFoundException) as err:
        await product_usecase.get(id=UUID("8217ca99-5d0c-40b8-bad8-dc5277accc83"))

    assert (
        err.value.message
        == "Product not found with filter: ID=8217ca99-5d0c-40b8-bad8-dc5277accc83"
    )
...
async def test_usecases_delete_should_return_not_found():
    with pytest.raises(NotFoundException) as err:
        await product_usecase.delete(id=UUID("8217ca99-5d0c-40b8-bad8-dc5277accc83"))
